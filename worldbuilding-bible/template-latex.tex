...
% Colors - define all colors first
\usepackage{xcolor}
\definecolor{trueblack}{HTML}{000000}  % Pure black for dark mode
\definecolor{nearblack}{HTML}{1A1A1A}  % Near-black for light mode text
\definecolor{pagewhite}{HTML}{FFFFFF}
\definecolor{gray}{HTML}{888888}
\definecolor{hotpink}{HTML}{E85A6B}    % Hot red-pink for dark page captions

% Dark mode toggle: set dark-mode: true in markdown frontmatter
% Colors adapt based on mode
$if(dark-mode)$
% DARK MODE - softblack pages (like title), white text
\definecolor{pagebg}{HTML}{0D0D0D}
\definecolor{textcolor}{HTML}{FFFFFF}
\definecolor{textmuted}{HTML}{AAAAAA}
\definecolor{boxbg}{HTML}{1A1A1A}
\definecolor{border}{HTML}{333333}
\definecolor{quotetint}{HTML}{1A1A1A}
\definecolor{linkcolor}{HTML}{888888}
$else$
% LIGHT MODE - light grey pages, dark text
\definecolor{pagebg}{HTML}{F8F8F8}
\definecolor{textcolor}{HTML}{1A1A1A}
\definecolor{textmuted}{HTML}{666666}
\definecolor{boxbg}{HTML}{EFEFEF}
\definecolor{border}{HTML}{DDDDDD}
\definecolor{quotetint}{HTML}{E8E4EB}
\definecolor{linkcolor}{HTML}{888888}
$endif$

% Softer black for title/architecture pages (between pure black and near-black)
\definecolor{softblack}{HTML}{0D0D0D}

% Length for title width (used to align subtitle)
\newlength{\titlewidth}
...
% H1 - PT Mono 24pt (Soviet technical aesthetic)
\titleformat{\section}
  {\subtitlefont\bfseries\fontsize{24pt}{28pt}\selectfont\color{textcolor}}
  {}{0em}{}
\titlespacing*{\section}{0pt}{8mm}{4mm}  % 8mm before, 4mm after major sections

% Page break before sections - DISABLED (use ::: {.new-page} in markdown instead)
\newcommand{\sectionbreak}{}

% H2 - PT Mono 14pt with flexible spacing
\titleformat{\subsection}
  {\subtitlefont\bfseries\fontsize{14pt}{17pt}\selectfont\color{textcolor}}
  {}{0em}{}
\titlespacing*{\subsection}{0pt}{4mm plus 1mm minus 0.5mm}{1.5mm}

% H3 - PT Mono Bold 11pt with flexible spacing
\titleformat{\subsubsection}
  {\subtitlefont\bfseries\fontsize{11pt}{13pt}\selectfont\color{textcolor}}
  {}{0em}{}
\titlespacing*{\subsubsection}{0pt}{3mm plus 1mm minus 0.5mm}{1mm plus 0.25mm}
...
\usepackage[colorlinks=true, linkcolor=textcolor, citecolor=textcolor, urlcolor=linkcolor, pdfusetitle]{hyperref}
...
\usepackage{caption}
\captionsetup{font={scriptsize,tt}, labelfont={bf}, format=plain, skip=0.5mm, justification=raggedright, singlelinecheck=false}
...
% Page-spanning columns: using multicols* for UNBALANCED fill (left-first, no balancing)
% multicols* fills columns sequentially without balancing at end of environment
...
\newcommand{\colpairfig}[4]{%
  \begin{figure}[H]%
    ...
    \begin{tabular}{@{}c@{\hspace{1mm}}c@{}}%
      \includegraphics[width=0.48\columnwidth]{#1} &
      \includegraphics[width=0.48\columnwidth]{#2} \\
    \end{tabular}%
    \caption[#4]{#3}%
  \end{figure}%
}

% 3-image horizontal row with single caption
...
\newcommand{\coldualfig}[4]{%
  \begin{figure}[H]%
    \centering%
    \includegraphics[width=\columnwidth]{#1}\\[1mm]
...
\renewcommand{\maketitle}{%
  ...
  % Soft black background using TikZ overlay (always black regardless of mode)
  ...
  \begin{center}
    % Title in Molot (white on black) - save width for subtitle alignment
    \settowidth{\titlewidth}{\titlefont\fontsize{90pt}{100pt}\selectfont\@title}%
    {\titlefont\fontsize{90pt}{100pt}\selectfont\color{white}\@title\par}

    \vspace{15mm}

    % Subtitle in PT Mono (white on black) - constrained to title width
    $if(subtitle)$
    {\subtitlefont\bfseries\fontsize{16pt}{20pt}\selectfont\color{white} Worldbuilding Bible\par}
    \vspace{4mm}
    \parbox{\titlewidth}{\centering\subtitlefont\fontsize{11pt}{15pt}\selectfont\color{white!60} $subtitle$\par}
    \vspace{10mm}
    $endif$

    % Author and date in PT Mono (white on black)
    {\subtitlefont\small\color{white} \@author\quad·\quad\@date\par}

    \vspace{10mm}

    % Institution in PT Mono (white on black)
    {\subtitlefont\small\color{white} Technische Hochschule Ostwestfalen-Lippe\\[2mm]
     Fachbereich Medien und Kultur, Master Medienproduktion\\[2mm]
     $if(subject)$Fach: $subject$$if(tutor)$ · Dozent: $tutor$$endif$$endif$\par}
  \end{center}

  \vfill

  % Logos at bottom right using TikZ overlay (white variants)
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=south east, inner sep=0pt] at ([xshift=-15mm, yshift=15mm]current page.south east) {%
      \includegraphics[height=20mm]{../../assets/logos/th-owl-logo-white.png}%
      \hspace{5mm}%
      \includegraphics[height=20mm]{../../assets/logos/mp-logo-solid-white.png}%
    };
  \end{tikzpicture}
...
$if(toc)$
% TOC page: softblack background like title page
\thispagestyle{empty}

% Softblack background
\begin{tikzpicture}[remember picture, overlay]
  \fill[softblack] (current page.south west) rectangle (current page.north east);
\end{tikzpicture}
...
\begin{minipage}[t]{0.48\textwidth}
  ...
  % TOC title in same style as other headings
  {\subtitlefont\bfseries\fontsize{24pt}{28pt}\selectfont\color{white} Inhaltsverzeichnis\par}
  \vspace{6mm}
  \color{white}
  \makeatletter
  \@starttoc{toc}
...
\begin{minipage}[t]{0.48\textwidth}
  \vspace{0pt}
  \raggedleft
  % Small creature image at top right
  \includegraphics[width=0.5\linewidth]{../figures/toc-illustration.png}

  \vspace{10mm}

  % Intro text block
  \raggedright
  {\subtitlefont\bfseries\fontsize{14pt}{18pt}\selectfont\color{white} Привет!\par}
  \vspace{4mm}
  {\small\color{white} Dies ist eine Worldbuilding Bible — ein Referenzdokument für eine fiktive Welt. Entstanden im Fach \textit{Virtuelle Welten} im Master Medienproduktion an der TH OWL.

  \vspace{3mm}

  Das Projekt verbindet Houdini und Unreal Engine: prozedural generierte Architektur trifft auf Echtzeit-Rendering. Das Ergebnis ist ПАНЕЛЬКИ — eine Stadt aus Schnee und Beton, bevölkert von Tieren in abgetragenen Mänteln.

  \vspace{3mm}

  Ein Konzept für eine kleine, melancholische Gamewelt.\par}
\end{minipage}
\newpage
$endif$
